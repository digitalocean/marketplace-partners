    ...
    if [ -d ${uhome}/ ]; then
            if [ -d ${uhome}/.ssh/ ]; then
                if  ls ${uhome}/.ssh/*> /dev/null 2>&1; then
				    ...
				    linesinauthkeysfile=$( cat ${key} | grep ssh | wc -l )	
				    if [ "${linesinauthkeysfile}" -gt 1 ]; then
					echo -en "\e[41m[FAIL]\e[0m User \e[1m${user}\e[0m has **${linesinauthkeysfile} KEYS(!!!)** in the authorized_keys file in \e[93m${key}\e[0m\n"
