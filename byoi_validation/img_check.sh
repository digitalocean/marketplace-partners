#!/bin/bash
#
# DigitalOcean BYOI Image Validation Tool
...
function checkLogs {
    echo "Checking for log files which may contain sensitive information or details you may not wish to share.  Please review any issues detected to ensure that the contents of these files are not sensitive."
    ...
    do
      ...
      if [[ "${u[0]}" == "${user}" ]]; then
        if [[ ${u[1]} == "!" ]] || [[ ${u[1]} == "*" ]]; then
            ...
            echo -en "\e[93m[WARN]\e[0m User ${user} has a password set on their account.  This password will be included on Droplets created from this image\n"
            ((WARN++))
            if [[ $STATUS != 2 ]]; then          
                STATUS=1
            fi
    ...
    if [ -d ${uhome}/ ]; then
            if [ -d ${uhome}/.ssh/ ]; then
                if  ls ${uhome}/.ssh/*> /dev/null 2>&1; then
                  echo "Checking for ssh-keys in user directories... authorized_keys files list keys that are allowed access to Droplets created from this image."
             ...
             if [ -f /root/.bash_history ];then
                      ...
                      if [[ $BH_S -lt 200 ]]; then
                          ...
                          echo -en "\e[93m[WARN]\e[0m ${user}'s Bash History should be cleared to prevent sensitive information from leaking\n"
                          ((WARN++))
        ...
        do
          ...
          if [[ "${u[0]}" == "${user}" ]]; then
              if [[ ${u[1]} == "!" ]] || [[ ${u[1]} == "*" ]]; then
                  ...
                  echo -en "\e[93m[WARN]\e[0m User ${user} has a password set on their account.\n"
                  ((WARN++))
                  if [[ $STATUS != 2 ]]; then
...
function checkCloudInit {
    ...
    if hash cloud-init 2>/dev/null; then
        ...
        CI="\e[93m[WARN]\e[0m No valid verison of cloud-init was found. Images without cloud-init are not officially supported.\n"
        ((WARN++))
        if [[ $STATUS != 2 ]]; then
...
clear
echo "DigitalOcean BYOI Image Validation Tool ${VERSION}"
echo "This script currently supports the following Linux distributions:"
echo " - Ubuntu 16.04 LTS"
echo " - Ubuntu 18.04 LTS"
echo " - CentOS 7.x"
echo "Checking local system for BYOI compatibility..."
...
if [[ $ost == 1 ]]; then
    ...
    echo -en "\e[41m[FAIL]\e[0m ${OS} is not a supported Operating System.  This utility is not intended for use with this operating system\n"
    ((FAIL++))
    STATUS=2
fi
...
if [[ $osv == 1 ]]; then
    ...
    echo -en "\e[41m[FAIL]\e[0m ${OS} ${VER} is not a supported Operating System Version. This utility has not been tested with this version of ${OS}.\n"
    ((FAIL++))
    STATUS=2
else
    echo "Exiting..."
...
echo -en "\n\n"
echo "Cloud-init is required for several features of the DigitalOcean cloud.  We strongly recommend using"
echo "a modern version of cloud init (0.77 or greater).  Versions as early as 0.75 will provide partial"
echo "support but some DigitalOcean products or features may be unavailable."

echo -en "\n${CI}\n"
if [[ $CI_VER ]]; then
echo -en "\n\nThis system has cloud init version: "
echo -en "\n\n\\e[36m${CI_VER}\e[0m\n\n"
else
echo en "\n\nCloud init was not detected.  Please install it with 'apt install cloud-init' (Ubuntu) or 'yum install cloud-init' (CentOS)"
fi
checkFirewall
...
if [[ $STATUS == 0 ]]; then
    ...
    echo -en "Please review all [WARN] items above and ensure they are intended or resolved.  If you do not have a specific requirement, we recommend resolving these items before using this image in production\n\n"
else
    echo -en "Some critical tests failed.  This means your image should be reviewed manually or this test should be re-run before use in production\n\n"
fi
